...
import { frontend_icon, backend_icon, tools_icon } from '../assets/images/SKILLS';
...
const Skills = () => (
  <section id='skills-section' className='bg-background'>
    <div name='inner' className='flex flex-col mx-[5%]'>
      ...
      <SkillsContainer title="Front End Development" skills={skills.FrontEnd} icon={frontend_icon} />
      <SkillsContainer title="Back End Development" skills={skills.BackEnd} icon={backend_icon} />
      <SkillsContainer title="Software Tools" skills={skills.Tools} icon={tools_icon} />
    </div>
  </section>
);
...
const SkillCard = ({ title, skills, icon }) => (
  <div className="flex flex-col w-full relative overflow-hidden pb-5 breakpoint:pb-16 laptop:pb-32 ">
    <Typewriter text={title} delay={0.05} className="mt-6 font-japan-bold relative text-sm breakpoint:text-md laptop:text-lg text-center tablet:text-start">
      <img src={icon} alt={icon} className="w-5 h-5 inline-block mr-2" />
    </Typewriter>
